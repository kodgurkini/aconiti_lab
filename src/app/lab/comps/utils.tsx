...
export const useRandomRange = (min: number, max: number, seed = 0) => {
  const [value] = useState(
    () => min + (Math.sin(seed) + 1) * 0.5 * (max - min)
  );
  return value;
};

export const LabTitleDesc = ({
  number,
  title,
  description,
}: {
    ...
    <>
      ...
      <p
        style={{
          ...
          fontWeight: "500",
          marginBottom:
            "8px" /* fontFamily: reenieBeanieRegular.style.fontFamily */,
        }}
      >
        {title}
      </p>
  ...
  return (
    ...
    >
      ...
      >
        {currentLab > 1 ? (
          <Link
            href={`/lab${String(currentLab - 1).padStart(3, "0")}`}
            style={{
              display: "flex",
              alignItems: "center",
              gap: "8px",
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Image src="/img/x-logo.svg" width={12} height={12} alt="x-logo" />
          <p
            style={{
              opacity: 0.64,
              fontSize: "11px",
